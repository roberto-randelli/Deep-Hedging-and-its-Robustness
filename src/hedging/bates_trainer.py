"""
Deep-hedging training loop for Bates model — He, Sutter & Gonon (2025).

BatesHedgeNet and p0 are jointly optimised with Adam + LR schedule.
Network input at each step: [log(S_t), V_t], shape (batch, N, 2).
Network output:             [δ_S_t, δ_V_t], shape (batch, N, 2).

Mirrors heston_trainer.py with Bates-specific imports.
"""
...
def train(
    ...
    *,
    ...
    """
    Jointly optimises BatesHedgeNet weights and p0 via Adam.

    LR schedule (mirrors heston_trainer.py):
        epochs   0–199  : lr × 1
        epochs 200–399  : lr × 0.1
        epochs 400–599  : lr × 0.01
        epochs 600–999  : lr × 0.001
        epochs 1000+    : lr × 0.0001

    Args:
        network:    BatesHedgeNet. Mutated in-place.
